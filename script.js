const portalDaten = {
  "genie": { "name": "Genie", "farbe": "#0b31ba", "kategorie": "buehne", "maschinen": {
      "GS1532 (Schere)": [
  ...
  "jlg": { "name": "JLG", "farbe": "#f0660a", "kategorie": "buehne", "maschinen": {
      "1932 (Schere)": [{"titel": "📘 Serviceanleitung", "datei": "pdf/JLG/1932/Serviceanleitung.pdf"}],
      "2032 (Schere)": [{"titel": "📘 Serviceanleitung", "datei": "pdf/JLG/2032/Serviceanleitung.pdf"}]
  }},
  "zoomlion": { "name": "Zoomlion", "farbe": "#56f00a", "kategorie": "buehne", "maschinen": {
      "Z0701 (Schere)": [{"titel": "📘 Serviceanleitung", "datei": "pdf/Zoomlion/Z0701/Serviceanleitung.pdf"}]
  }},
  "haulotte": { "name": "Haulotte", "farbe": "#f0c60a", "kategorie": "buehne", "maschinen": {
      "Star 10 (Telekopmast)": [{"titel": "📘 Serviceanleitung", "datei": "pdf/Haulotte/Star10/Serviceanleitung.pdf"}]
  }},
  "dingli": { "name": "Dingli", "farbe": "#f0660a", "kategorie": "buehne", "maschinen": {
      "BA28BERT (Gelenk-Teleskop)": [{"titel": "📘 Betriebsanleitung", "datei": "pdf/Dingli/BA28BERT/Betriebsanleitung.pdf"}]
  }},
  "manitou": { "name": "Manitou", "farbe": "#c2160a", "kategorie": "stapler", "maschinen": {
  ...
  "merlo": { "name": "Merlo", "farbe": "#51f505", "kategorie": "stapler", "maschinen": {
      "TS0625 (Teleskopstapler)": [
  ...
  "teupen": { "name": "Teupen", "farbe": "#ba0924", "kategorie": "stapler", "maschinen": {
  ...
  "skyjack": { "name": "Skyjack", "farbe": "#f55505", "kategorie": "buehne", "maschinen": {
      "SJ12 (Roll-Lift)": [
...
let navigationsVerlauf = [];

// NEU: Zeigt zuerst die Kategorien an
function zeigeKategorienUebersicht() {
  ...
  let html = `
    ...
    <div class="grid">
      <button class="btn kat-btn" onclick="zeigeHerstellerNachKategorie('buehne')">🚀 Arbeitsbühnen</button>
      <button class="btn kat-btn" onclick="zeigeHerstellerNachKategorie('stapler')">🚜 Stapler & Kettenbühnen</button>
    </div>
  `;
  app.innerHTML = html;
}

// Zeigt Hersteller gefiltert nach gewählter Kategorie
function zeigeHerstellerNachKategorie(katId) {
  navigationsVerlauf.push({ typ: 'kategorie', wert: katId });
  backBtn.style.display = 'block';
  homeBtn.style.display = 'block';

  const titel = katId === 'buehne' ? 'Arbeitsbühnen' : 'Stapler & Kettenbühnen';
  let html = `<h2>Hersteller (${titel})</h2><div class="grid">`;
  
  for (const key in portalDaten) {
    const hersteller = portalDaten[key];
    if (hersteller.kategorie === katId) {
      html += `
        <button class="btn hersteller-btn" style="border-left-color: ${hersteller.farbe}" onclick="waehleHersteller('${key}')">
          🏢 ${hersteller.name}
        </button>
      `;
    }
...
function waehleHersteller(key) {
  navigationsVerlauf.push({ typ: 'hersteller', wert: key });
  backBtn.style.display = 'block';
  homeBtn.style.display = 'block';
  
  const hersteller = portalDaten[key];
  let html = `<h2>${hersteller.name} - Modelle</h2><div class="grid">`;
  
  for (const maschine in hersteller.maschinen) {
    html += `
      <button class="btn maschine-btn" onclick="waehleMaschine('${key}', '${maschine}')">
        📦 ${maschine}
      </button>
    `;
  }
...
function waehleMaschine(herstellerKey, maschineKey) {
  navigationsVerlauf.push({ typ: 'maschine', hersteller: herstellerKey, maschine: maschineKey });
  
  const dokumente = portalDaten[herstellerKey].maschinen[maschineKey];
  let html = `<h2>${portalDaten[herstellerKey].name} ${maschineKey}</h2><div class="grid">`;
  
  if (!dokumente || dokumente.length === 0) {
    html += '<p>Keine Dokumente für diese Maschine hinterlegt.</p>';
  } else {
    dokumente.forEach(doku => {
      // HIER GEÄNDERT: Klasse von "doku-btn" zu "maschine-btn" gewechselt
      html += `
...
// NEU: Funktionen zur Steuerung der PDF-Direktanzeige im Portal (Modal)
function oeffnePdf(dateiPfad, titel) {
