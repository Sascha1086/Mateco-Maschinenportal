...
let aktuelleMaschine = null;
let navigationsVerlauf = [];

// Initialisiert die Startansicht
function zeigeHerstellerUebersicht() {
  aktuellerHersteller = null;
  aktuelleMaschine = null;
  navigationsVerlauf = [];
...
function waehleMaschine(herstellerKey, maschineKey) {
  ...
  aktuelleMaschine = maschineKey;
  
  const dokumente = portalDaten[herstellerKey].maschinen[maschineKey];
  let html = `<h2>${herstellerKey.toUpperCase()} ${maschineKey} - Dokumente</h2><div class="grid">`;
  ...
  if (dokumente.length === 0) {
    ...
    dokumente.forEach(doku => {
      html += `
        <a href="${doku.datei}" target="_blank" class="btn doku-btn">
          ${doku.titel}
        </a>
      `;
    });
...
// Dynamische Live-Suche über Hersteller & Maschinenmodelle
function sucheMaschine() {
  const suchbegriff = searchInput.value.toLowerCase().trim();
  if (suchbegriff === "") {
    zeigeHerstellerUebersicht();
    return;
  }

  // Bei aktiver Suche Buttons ausblenden, um Verwirrung zu vermeiden
  backBtn.style.display = 'none';
  homeBtn.style.display = 'none';
  ...
  for (const hKey in portalDaten) {
    const hersteller = portalDaten[hKey];
    
    // 1. Treffer bei Herstellernamen
    if (hersteller.name.toLowerCase().includes(suchbegriff)) {
      html += `
